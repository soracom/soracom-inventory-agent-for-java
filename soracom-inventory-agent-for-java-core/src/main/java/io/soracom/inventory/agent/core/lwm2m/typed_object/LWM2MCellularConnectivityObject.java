 ...
 **/
@LWM2MObject(objectId = 10, name = "LWM2M Cellular Connectivity")
public abstract class LWM2MCellularConnectivityObject extends AnnotatedLwM2mInstanceEnabler {
	...
	/**
	 * Time period in MINUTES for which the device shall disconnect from cellular radio (PS detach, CS detach if applicable).
	...
	/**
	 * Applicable only when Signalling Radio Bearers are used to transfer data, Data over NAS (DoNAS), it indicates the maximum number of allowed uplink PDU transmissions per 6 minute interval aggregated across all PDN connections. See [3GPP-TS_23.401], octet 3 to 4 of the Serving PLMN rate control IE.
	 **/
	...
	/**
	 * Links to instances of the “APN connection profile” object representing every APN connection profile that has an activated PDN connection.
	 **/
	@Resource(resourceId = 11, operation = Operation.Read, multiple = true)
	public abstract ObjectLink readActivatedProfileNames();
}
