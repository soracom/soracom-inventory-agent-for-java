 ...
 **/
@LWM2MObject(objectId = 11, name = "LWM2M APN Connection Profile", multiple = true)
public abstract class LWM2MAPNConnectionProfileObject extends AnnotatedLwM2mInstanceEnabler {
	...
	/**
	 * If this resource is present for a connection profile, it enables the device to choose an APN according to a device specific algorithm. It provides a fall-back mechanism e.g. for some MVNO SIMs the configured APN may not work.  Resource not included or set to False in case the ‘APN’ resource is specified.
	 **/
	...
	/**
	 * Allows the profile to be remotely enabled or disabled. True: APN profile is enabled. False: APN profile is disabled. If the status is set to disabled (False) while a data connection for that profile is active, that data connection will be released.
	 **/
