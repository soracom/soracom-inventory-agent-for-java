 ...
 * on which to attach/register and what type of bearer to then connect. This
 * object allows via remote bearer and network configuration to overwrite
...
public abstract class BearerSelectionObject extends AnnotatedLwM2mInstanceEnabler {
	 ...
	 * management procedures: 0: auto connect 1: 3GPP PS preferred 2: 3GPP PS GSM
	 * (GPRS) preferred (including EC-GSM-IoT) 3: 3GPP PS UMTS preferred 4: 3GPP PS
	 * LTE preferred 5: 1xEV-DO preferred (1) 6: 3GPP CS preferred (1) 7: WLAN
	 * preferred 8: Ethernet preferred (1) 9: DSL preferred (1) 10: Bluetooth
	 * preferred (1) 11: WIMAX preferred (1) 12: 3GPP PS LTE with CIoT EPS
	 * optimisations, User Plane preferred (2) 13: 3GPP PS LTE with CIoT EPS
	 * optimisations, Control Plane preferred (2)14: 3GPP PS NB-IoT Control Plane
	 * optimisations preferred (2) 15: 3GPP PS NB-IoT User Plane optimisations
	 * preferred (2) 16-100: Reserved for future use The Preferred Communications
	 * Bearer resource specifies the preferred communications bearer that the LWM2M
	 * Client is requested to use for connecting to the LWM2M Server. If multiple
	 * preferred communications bearers are specified, the priority order is
	 * reflected by the resource instance order. E.g. the bearer which appears first
	 * in the list of resource instances is to have higher priority over the rest of
	 * available bearers. The LWM2M Client SHOULD use the preferred bearers with
	 * higher priority first if they are available. If none of indicated preferred
	 * bearers is available, the LWM2M Client SHOULD wait until one of them becomes
	 * available.
	 **/
	...
	/**
	 * Comma separated list of MCC+MNC of operators, in priority order. Resource
	 * “operator list mode” indicates how to process this list.
	 **/
	 ...
	 * list (white list), or, the preferred operator list. 0=preferred 1=allowed
	 **/
